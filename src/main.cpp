...
static std::unordered_map<size_t, std::string_view> const&
getImplementedTypes()
{
  static std::unordered_map<size_t, std::string_view> const k_type_checker{
      ...
      { kUInt8Hash,  "uint8_t"},
      {kUInt16Hash, "uint16_t"},
      {kUInt32Hash, "uint32_t"},
      {kUInt64Hash, "uint64_t"},
  };
  return k_type_checker;
}

static std::unordered_map<size_t, MenuOptions> const&
getMenuOptions()
{
...
class Settings {
 ...
 public:
  ...
  void
  setTypeHash(size_t hash)
  {
    _type_hash = hash;
  }
  void
  setTypeEnum(EnumTypes type)
  {
    _type_enum = type;
  }
  void
...
std::ostream&
operator<<(std::ostream& out, Settings const& settings)
{
  out << "========================\n";
...
template <size_t N>
static void
enterVector(PolymorpicVector<N>& vector, Settings const& settings)
{
  clearScreen();
  ...
  while (index < N) {
    ...
    try {
      switch (current_type) {
        case EnumTypes::Int:
          spare_vector[index]
              .template emplace<static_cast<size_t>(EnumTypes::Int)>(
                  stoi(string_input));
          break;
        case EnumTypes::Float:
          spare_vector[index]
              .template emplace<static_cast<size_t>(EnumTypes::Float)>(
                  stof(string_input));
          break;
        case EnumTypes::Double:
          spare_vector[index]
              .template emplace<static_cast<size_t>(EnumTypes::Double)>(
                  stod(string_input));
          break;
        case EnumTypes::Char:
          spare_vector[index]
              .template emplace<static_cast<size_t>(EnumTypes::Char)>(
                  string_input[0]);
          break;
        case EnumTypes::String:
          spare_vector[index]
              .template emplace<static_cast<size_t>(EnumTypes::String)>(
                  string_input);
          break;
        case EnumTypes::Bool: {
          ...
          if (hashed_input != kTrueHashSymbolic &&
            ...
            value_to_set = stoi(string_input) == 1;
          }
          spare_vector[index]
              .template emplace<static_cast<size_t>(EnumTypes::Bool)>(
                  value_to_set);
          break;
        }
        case EnumTypes::Int8:
          spare_vector[index]
              .template emplace<static_cast<size_t>(EnumTypes::Int8)>(
                  static_cast<int8_t>(stoi(string_input)));
          break;
        case EnumTypes::Int16:
          spare_vector[index]
              .template emplace<static_cast<size_t>(EnumTypes::Int16)>(
                  static_cast<int16_t>(stoi(string_input)));
          break;
        case EnumTypes::Int32:
          spare_vector[index]
              .template emplace<static_cast<size_t>(EnumTypes::Int32)>(
                  static_cast<int32_t>(stol(string_input)));
          break;
        case EnumTypes::Int64:
          spare_vector[index]
              .template emplace<static_cast<size_t>(EnumTypes::Int64)>(
                  static_cast<int64_t>(stoll(string_input)));
          break;
        case EnumTypes::UInt8:
          spare_vector[index]
              .template emplace<static_cast<size_t>(EnumTypes::UInt8)>(
                  static_cast<uint8_t>(stoi(string_input)));
          break;
        case EnumTypes::UInt16:
          spare_vector[index]
              .template emplace<static_cast<size_t>(EnumTypes::UInt16)>(
                  static_cast<uint16_t>(std::stoull(string_input)));
          break;
        case EnumTypes::UInt32:
          spare_vector[index]
              .template emplace<static_cast<size_t>(EnumTypes::UInt32)>(
                  static_cast<uint32_t>(std::stoull(string_input)));
          break;
        case EnumTypes::UInt64:
          spare_vector[index]
              .template emplace<static_cast<size_t>(EnumTypes::UInt64)>(
                  static_cast<uint64_t>(std::stoull(string_input)));
          break;
      }
    }
    catch (std::exception& e) {
...
static std::unordered_map<size_t, EnumTypes> const&
getEnumType()
{
  static std::unordered_map<size_t, EnumTypes> const enum_types_converter{
      {   kIntHash,    EnumTypes::Int},
      { kFloatHash,  EnumTypes::Float},
      {kDoubleHash, EnumTypes::Double},
      {  kCharHash,   EnumTypes::Char},
      {kStringHash, EnumTypes::String},
      {  kBoolHash,   EnumTypes::Bool},

      {  kInt8Hash,   EnumTypes::Int8},
      { kInt16Hash,  EnumTypes::Int16},
      { kInt32Hash,  EnumTypes::Int32},
      { kInt64Hash,  EnumTypes::Int64},

      { kUInt8Hash,  EnumTypes::UInt8},
      {kUInt16Hash, EnumTypes::UInt16},
      {kUInt32Hash, EnumTypes::UInt32},
      {kUInt64Hash, EnumTypes::UInt64},
  };
  return enum_types_converter;
}

static void
changeType(Settings& settings)
{
  clearScreen();
  std::string string_input;
  while (true) {
